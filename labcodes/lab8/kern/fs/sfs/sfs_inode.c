...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2012011359 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
	if ((blkoff = offset % SFS_BLKSIZE) != 0){
		...
		if (sfs_bmap_load_nolock(sfs,sin,blkno,&ino) != 0)
			goto out;
		if (sfs_buf_op(sfs,buf,size,ino,blkoff) != 0)
			goto out;
		alen += size;
		if (nblks == 0)
			goto out;
	...
	if (size != 0)	{
		if ( sfs_bmap_load_nolock(sfs, sin, blkno, &ino) != 0)
            goto out;
        if (sfs_buf_op(sfs, buf, size, ino, 0) != 0)
            goto out;
        alen += size;
	}
out:
